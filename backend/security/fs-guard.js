...
// const sharp = require('sharp'); // optional, but recommended to verify images are real

// async function resolveUnder(baseDir, candidateRelativePath) {
//   if (typeof candidateRelativePath !== 'string') throw new Error('Path must be a string');
//   if (candidateRelativePath.includes('\0')) throw new Error('NUL byte in path');
//   if (path.isAbsolute(candidateRelativePath) || /^[a-zA-Z]+:\/\//.test(candidateRelativePath)) {
//     throw new Error('Absolute paths and URLs are not allowed');
//   }
//   // Normalize POSIX-style first (frontends often send / even on Windows)
//   const normalizedPosix = candidateRelativePath.replace(/\\/g, '/'); // allow Windows-style input
//   const normalized = path.posix.normalize(normalizedPosix).replace(/^(\.\/)+/, '');
//   if (normalized.startsWith('..')) throw new Error('Path traversal detected');

//   const baseReal = await fsp.realpath(baseDir);
//   const full = path.join(baseReal, normalized.split('/').join(path.sep));
//   const fullReal = await fsp.realpath(full).catch(async (e) => {
//     if (e && e.code === 'ENOENT') {
//       const parentReal = await fsp.realpath(path.dirname(full));
//       if (!parentReal.startsWith(baseReal + path.sep) && parentReal !== baseReal) {
//         throw new Error('Parent directory escapes base');
//       }
//       return full; // allow non-existent at this stage if caller wants
//     }
//     throw e;
//   });

//   if (fullReal !== baseReal && !fullReal.startsWith(baseReal + path.sep)) {
...
//   await sharp(absPath).metadata(); // throws if not an image or corrupt
// }
